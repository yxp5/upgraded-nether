...
public class FieryIngot extends Item {

    public FieryIngot(Settings settings) {
        super(settings);
    }
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        // Basic Info
        tooltip.add(new TranslatableText("A hot ingot used by adventurers to make hotter equipments"));

        if (Screen.hasShiftDown()) {
            tooltip.add(new TranslatableText("Tier 2 Material").formatted(Formatting.YELLOW));
        } else {
